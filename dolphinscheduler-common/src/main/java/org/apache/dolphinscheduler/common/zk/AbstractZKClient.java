...
public abstract class AbstractZKClient {
    ...
    private String createZNodePath(ZKNodeType zkNodeType) throws Exception {
        ...
        String registerPath = zkClient
                ...
                .forPath(serverPathPrefix + "_", heartbeatZKInfo.getBytes());
    ...
    public boolean checkZKNodeExists(String host, ZKNodeType zkNodeType) {
        ...
        if (StringUtils.isEmpty(path)) {
            logger.error("check zk node exists error, host:{}, zk node type:{}", host, zkNodeType);
            return false;
        }
    ...
    protected void initSystemZNode() {
        try {
            //创建 /dolphinscheduler/masters 持久节点 保存 所有master信息
            createNodePath(getMasterZNodeParentPath());
            //创建 /dolphinscheduler/workers 持久节点， 保存所有worker信息
            createNodePath(getWorkerZNodeParentPath());
            // 创建 dolphinscheduler/dead-servers 持久节点，保存所有挂掉的节点信息
